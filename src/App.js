...
export default function TrustTypeLogicApp() {
  ...
  const handleAnswer = (value) => {
    ...
    setStep(step < questions.length - 1 ? step + 1 : 'result');
  };

  const determineTrustType = () => {
    const { control, asset_protection, estate_tax, medicaid, gifting, control_goals } = answers;
    if (asset_protection === 'yes' || estate_tax === 'yes' || medicaid === 'yes' || gifting === 'yes') return 'Irrevocable Trust';
    if (control === 'yes' && control_goals === 'yes') return 'Revocable Living Trust';
    return 'Further clarification needed – consider speaking with a trust advisor.';
  };

  const downloadSummary = () => {
    const summary = `Trust Type Logic Summary\n\n` + questions.map(q => `${q.text}\nAnswer: ${answers[q.id] || 'N/A'}\n\n`).join('') + `Recommended Trust Type: ${determineTrustType()}`;
    ...
    <div className="p-4 max-w-xl mx-auto">
      ...
      {step === 'result' ? (
        <div>
          ...
          <button onClick={downloadSummary} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded">Download Summary</button>
        </div>
      ) : (
        <div>
          <p className="text-lg mb-4">{questions[step].text}</p>
          <div className="flex space-x-4">
            <button onClick={() => handleAnswer('yes')} className="px-4 py-2 bg-green-600 text-white rounded">Yes</button>
            <button onClick={() => handleAnswer('no')} className="px-4 py-2 bg-red-600 text-white rounded">No</button>
          </div>
        </div>
      )}
    </div>
